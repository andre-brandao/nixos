...
TEMPLATES=$(echo "$FLAKE_JSON" | jq -r '
  .templates // {} |
  to_entries |
...
echo ""
SELECTED=$(echo "$TEMPLATE_LIST" | gum choose --header "Select a template:")
...
if ! gum confirm "Do you want to continue? ($TEMPLATE_NAME)"; then
