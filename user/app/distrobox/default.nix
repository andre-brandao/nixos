...
let
  ...
  mkBoxLinks = box:
    ...
    in listToAttrs (map (link: {
      name = "${box.home}/${link}";
      value = { source = ln "${config.home.homeDirectory}/${link}"; };
    }) links);

  mkBoxAlias = let db = "${pkgs.distrobox}/bin/distrobox";
  in box:
...
in {
  home.packages = let aliases = mapAttrs (name: value: mkBoxAlias value) boxes;
  in (attrValues aliases) ++ [ pkgs.distrobox ];

  home.file = let links = mapAttrs (name: value: mkBoxLinks value) boxes;
